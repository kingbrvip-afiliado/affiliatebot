...
const config = {
sources: {
telegram: ["@wolf_ofertas","@achadinhosafiliadosss","@clubedeofertasss","@gatunopromos","
twitter: ["@xetdaspromocoes","@achadinhos_dadu","@achadinhodasho","@acheinaxo","@capivar
},
targets: {
twitterEnabled: true,
...
const postExists = (id, t) => !!db.prepare('SELECT 1 FROM posts WHERE id=? AND source_type=?'
function savePost(p) {
db.prepare(`
...
const getPost = (id, t) => db.prepare('SELECT * FROM posts WHERE id=? AND source_type=?').get
...
const getBotState = (c) const clearState = (c) => db.prepare('SELECT * FROM bot_state WHERE chat_id=?').get
=> db.prepare('DELETE FROM bot_state WHERE chat_id=?').run(S
const setBotState = (c, s, p=null, t=null) =>
db.prepare(`INSERT INTO bot_state (chat_id,state,pending_post_id,pending_source_type,update
...
const fmtBRL = (v) => v != null ? v.toLocaleString('pt-BR', { style: 'currency', currency: 'B
...
const ML_PATTERNS = /mercadolivre\.com\.br|mercadolibre\.com|meli\.st|meli\.la|meli\.sto
...
return { originalPrice: hi, salePrice: lo, discount: discount || Math.round(((hi - lo) /
}
...
// ─── Formatter ────────────────────────────────────────────────
const OPENERS = [' ACHADO DO DIA',' CORRE QUE TÁ BARATO',' OFERTA IMPERDÍVEL',' TÁ N
const INTROS = ['Saindo por só','Tá saindo a','Por apenas','Você leva por','Pode comprar por
const DISC_FN = [(d)=>`${d}% de desconto!`,(d)=>`${d}% OFF, não é brincadeira!`,(d)=>`você ec
const CTAS = [' Pega o seu aqui:',' Link direto:',' Garante o seu agora:',' Aprov
...
const OPENERS_TW = [
' IMPERDÍVEL ', ' CORRE ', ' QUE PREÇO ',
' OFERTA URGENTE ', ' TÁ BARATO ', ' ACHADO DO DIA ',
' OLHA ESSE PREÇO ', ' NÃO PASSA NÃO ',
];
...
fromTo = `De R$${fmtBRL(origPrice).replace('R$','').trim()} | Por R$${fmtBRL(price).repla
...
const price = salePrice ? `\n ${fmtBRL(salePrice)}` + (post.discount ? ` (*${post.discoun
return [
...
if (!url) return null;
if (/mercadolivre\.com\.br|mercadolibre\.com|meli\.st|meli\.la|meli\.store|click\.mlcdn/i.t
try {
const r = await axios.get('https://api.mercadolibre.com/link-building', {
params: { tracking_id: config.affiliates.mercadolivre.trackingId, url }, timeout: 800
});
if (r.data?.url || r.data?.link) return r.data.url || r.data.link;
} catch (_) {}
try {
const u = new URL(url); u.searchParams.set('tracking_id', config.affiliates.mercadolivr
} catch (_) { return url + '?tracking_id=' + config.affiliates.mercadolivre.trackingId; }
}
if (/shopee\.com\.br|shope\.ee/i.test(url)) {
try {
const u = new URL(url); u.searchParams.set('af_id', config.affiliates.shopee.affiliateI
...
if (imageUrl) {
try {
await pubBot.sendPhoto(g, imageUrl, { caption: text, parse_mode: parseMode return true;
} catch (_) {}
});
}
await pubBot.sendMessage(g, text, { parse_mode: parseMode, disable_web_page_preview:
return true;
};
try {
...
if (imageUrl) {
try { await pubBot.sendPhoto(g, imageUrl, { caption: plain }); results.push({ g, ok
}
...
try {
if (post.imageUrl) {
try { await approvalBot.sendPhoto(ADMIN, post.imageUrl, { caption: text, parse_mode: 'M
}
await approvalBot.sendMessage(ADMIN, text, { parse_mode: 'Markdown', reply_markup: } catch (e) { console.error('[BOT] Erro ao enviar aprovação:', e.message); }
kbd })
}
...
await approvalBot.sendMessage(chatId, ' *Manda seu link de afiliado* (ou `.` para usar
return;
}
if (action === 'ch') {
await doPublish(chatId, parts[1], parts[2], parts[3].includes('T1'), parts[3].includes('G
}
});
approvalBot.on('message', async (msg) => {
const chatId = String(msg.chat.id);
if (chatId !== String(ADMIN)) { if (msg.text === '/start') await approvalBot.sendMessage(ch
...
updatePost(state.pending_post_id, state.pending_source_type, 'approved', { affiliateLink: a
...
btns.push([{ text: '𝕏 Só Twitter', callback_data: `ch:${pid}:${ptype}:T1G0` }, { text: '✈
} else if (hasT) {
...
await approvalBot.sendMessage(chatId, ' *Onde publicar?*', { parse_mode: 'Markdown', repl
...
if (!post) { await approvalBot.sendMessage(chatId, ' Post não encontrado.'); return; }
...
await approvalBot.sendMessage(chatId, ' } else if (cmd === '/status') {
*AffiliateBot*\n\n`/status` — Estatísticas\n`/c
const s = getStats();
await approvalBot.sendMessage(chatId, ` *Stats*\n\n Total: ${s.total}\n Pendentes:
} else if (cmd === '/config') {
await approvalBot.sendMessage(chatId, ` *Config atual*\n\n Telegram: ${config.sources
} else if (cmd === '/cancel') {
clearState(chatId);
await approvalBot.sendMessage(chatId, ' Cancelado.');
}
}
// ─── Main loop ────────────────────────────────────────────────
...
if (missing.length) { console.error(' Variáveis ausentes:', missing.join(', ')); process.
console.log(`
╔══════════════════════════════════════════╗
