...
db.exec(`CREATE TABLE IF NOT EXISTS posts ( id TEXT NOT NULL, source_type TEXT NOT NULL, source_profile TEXT, raw_content TEXT, product_name TEXT, original_link TEXT, image_url TEXT, original_price REAL, sale_price REAL, discount INTEGER, status TEXT NOT NULL DEFAULT 'pending', affiliate_link TEXT, captured_at TEXT NOT NULL, published_at TEXT, PRIMARY KEY (id, source_type) ); CREATE TABLE IF NOT EXISTS bot_state ( chat_id TEXT PRIMARY KEY, state TEXT NOT NULL, pending_post_id TEXT, pending_source_type TEXT, updated_at TEXT NOT NULL );`);
console.log(’[DB] Banco inicializado.’);

const postExists = (id, t) => !!db.prepare(‘SELECT 1 FROM posts WHERE id=? AND source_type=?’).get(id, t);

function savePost(p) {
db.prepare(`INSERT OR IGNORE INTO posts (id, source_type, source_profile, raw_content, product_name, original_link, image_url, original_price, sale_price, discount, captured_at) VALUES (@id, @sourceType, @sourceProfile, @rawContent, @productName, @originalLink, @imageUrl, @originalPrice, @salePrice, @discount, @capturedAt)`).run({
id:            p.id,
...
db.prepare(`UPDATE posts SET status         = @status, affiliate_link = COALESCE(@afl, affiliate_link), published_at   = CASE WHEN @status = 'published' THEN @now ELSE published_at END WHERE id = @id AND source_type = @t`).run({ id, t, status, afl: extra.affiliateLink || null, now: new Date().toISOString() });
}
...
db.prepare(`INSERT INTO bot_state (chat_id,state,pending_post_id,pending_source_type,updated_at) VALUES (@c,@s,@p,@t,@n) ON CONFLICT(chat_id) DO UPDATE SET state=excluded.state, pending_post_id=excluded.pending_post_id, pending_source_type=excluded.pending_source_type, updated_at=excluded.updated_at`)
...
const ml     = text.match(/https?://[^\s<>”]*(?:mercadolivre.com.br|mercadolibre.com|meli.store|meli.st|click.mlcdn.com.br|produto.mercadolivre|shopee.com.br|mercadopago)[^\s<>”]*/gi)?.filter(u => /mercadolivre|mercadolibre|meli.|click.mlcdn/i.test(u)) || [];
const shopee = text.match(/https?://[^\s<>”]*(?:shopee.com.br|shope.ee|s.shopee.com.br|shopee.com/)[^\s<>”]*/gi) || [];
...
return [
`🆕 *Nova oferta capturada!*`, `, `📌 *Fonte:* ${src} — ${sourceProfile}`, productName ? `📦 *Produto:* ${productName}` : null, price || null, `,
`🔗 *Link original:*`,
originalLink || ‘*(não detectado)*’, ``,
...
} catch (*) { return url + ‘?tracking_id=’ + config.affiliates.mercadolivre.trackingId; }
}
...
try {
const client = new TwitterApi({ appKey: process.env.TWITTER_API_KEY, appSecret: process.env.TWITTER_API_SECRET, accessToken: process.env.TWITTER_ACCESS_TOKEN, accessSecret: process.env.TWITTER_ACCESS_SECRET });
...
const res  = await publishTwitter(text);
...
console.log(` ╔══════════════════════════════════════════╗ ║       🤖  AffiliateBot  iniciado!        ║ ╠══════════════════════════════════════════╣ ║  📡 Telegram : ${String(config.sources.telegram.length + ' canais').padEnd(26)}║ ║  🐦 Twitter  : ${String(config.sources.twitter.length + ' perfis').padEnd(26)}║ ║  📣 Publicando em @KingBRVip10 + X       ║ ╚══════════════════════════════════════════╝`);
